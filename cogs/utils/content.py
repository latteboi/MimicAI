...
HELP_CATEGORIES = {
    "1. Getting Started": {
        ...
        "API Keys and Where They Apply": (
            ...
            "Ming then Recraft for images, Fish Audio then Deepgram for speech -- the free ones free while your free quota lasts. "
            "Grounding is the exception: only Google runs the search tool, so it has no Final Fallback."
        ),
    ...
    "4. Memory": {
        ...
        "How Long-Term Memory Works": (
            ...
            "injected as `<archive_context>`, up to the LTM context size. New profiles start with this off; turning Auto-Creation on turns it "
    ...
    "6. Tools and Media": {
        "Web Grounding": (
            ...
            "Works on Google and OpenRouter; Ollama cannot carry it.\n"
            ...
            "whether a search would help; it usually says no, and you pay for the question either way. Works with every provider, "
            "and it is the only mode Ollama has.\n\n"
            "Prefer **RAG** unless the profile runs on Ollama. The two differ in who decides, not in what searching costs when it happens.\n\n"
        ...
        "URL Context": (
            ...
            "Native hands the whole prompt to Google, which decides for itself which links to open, file contents included.\n\n"
...
DEFAULT_HELP_DOCS = {
    ...
    "apis/openrouter.txt": (
        ...
        "- Symptom: 'My Claude profile is hallucinating web links' or 'Grounding failed with Claude'. Fix: You must set your Grounding mode to 'RAG' or 'Legacy RAG'. Native grounding only works with Google.\n"
    ...
    "memory/ltm_archive.txt": (
        ...
        "Settings: Auto-Creation, Auto-Recall and Memory Search are three switches on one screen at `/profile manage` -> Memory -> Memory Settings, with Creation Interval, Summarisation Context, Recall Context Size and Relevance Threshold behind its Parameters button.\n"
        "Creation: The bot tracks the exchange volume. Once the 'Creation Interval' is met, an auxiliary model extracts the recent conversation and condenses it into a third-person factual summary. Toggle this off from 'Auto-Creation' if you would rather curate by hand.\n"
    ...
    "memory/training_examples.txt": (
        ...
        "Management: `/profile manage` -> Training -> Manage Training Examples. Tune matching with 'Set Training Parameters'.\n"
    ...
    "features/image_generation.txt": (
        ...
        "- Symptom: '!image does nothing.' Fix: Image generation is disabled for that profile, or its image slot holds something that cannot draw -- a text model or Ollama. Choose a Google or OpenRouter image model under Set Models -> Image Generation.\n"
    ...
    "features/speech_tts.txt": (
        ...
        "Configuration: The Director's Desk (`/profile manage` -> Audio -> TTS Instructions) takes plain English for Vocal Archetype, Accent, Dynamics, and Pacing and Style.\n"
        "Voice: `/profile manage` -> Audio -> Choose TTS Voice offers the voices of the profile's speech model. On a Gemini model that is all thirty prebuilt voices, grouped by gender (14 female, 16 male) and described by Google's own one-word character (Kore is firm, Enceladus breathy, Sulafat warm); on an OpenRouter model, the voices that model lists, a page at a time. It is a dropdown rather than a text box because an unrecognised voice name is rejected by the API. A voice the model does not carry, such as one kept from another model, is swapped for the Gemini default or the OpenRouter model's first voice.\n"
        ...
        "- Symptom: 'No audio is generated.' Fix: The warning under the reply names the reason. Most often the server has no key for the speech model's provider -- Google for a Gemini speech model, OpenRouter for an OpenRouter one; the profile's own text model does not matter. Ollama has no speech models.\n"
    ...
    "features/grounding_and_rag.txt": (
        "Concept: Web Grounding enables real-time Google Search. URL Context lets the bot read links posted in chat. Both are set at `/profile manage` -> Tools. Grounding offers OFF, RAG, NATIVE and LEGACY RAG; URL Context offers OFF, NATIVE and RAG.\n"
        ...
        "RAG Mode (grounding): The character calls a `search_web` function when a reply needs a fact it cannot be sure of, and a background Google model runs that one query and reports back. Costs nothing on the turns it does not search, and sends the researcher one query rather than the conversation. Needs a provider that carries function declarations -- Google or OpenRouter, not Ollama -- and a Google API key on the server for the search itself.\n"
        "Legacy RAG Mode (grounding): The behaviour RAG used to have. Before every round a background model is given the recent transcript and asked whether a search is warranted; if it says yes it searches and summarises. Entirely model-agnostic, so it is the only grounding an Ollama profile can use. Costs one extra API call per message whether or not anything is searched.\n"
        ...
        "Troubleshooting / Symptoms:\n"
        "- Symptom: 'Grounding does nothing on my OpenRouter profile.' Fix: Switch it from NATIVE to RAG. Native tools are Google-only.\n"        "- Symptom: 'Grounding does nothing on my Ollama profile.' Fix: Ollama can carry neither NATIVE nor RAG. Set it to LEGACY RAG, which runs entirely on the server side.\n"
