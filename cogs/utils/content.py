...
HELP_CATEGORIES = {
    ...
    "6. Tools and Media": {
        "Web Grounding": (
            ...
            "This grounds replies only. Images have their own **Search grounding** in Media -> Set Image Output."
        ),
        "URL Context": (
        ...
        "Image Generation": (
            ...
            "**Output** (`/profile manage` -> Media -> Set Image Output) sets aspect ratio, resolution, quality, thinking level and search "
            ...
            "which grounds replies only. *RAG* works with any image model: a background model searches and its visual summary joins the "
            "prompt, at one extra call per image. *Web search* lets a Gemini model look facts up itself (a current logo, today's weather), and "
            ...
            "**Sampling** (Media -> Set Image Sampling) sets temperature, Top P and Top K for the image model specifically -- separate keys from "
        ...
        "Speech and the Director's Desk": (
            ...
            "**Director's Desk** (`/profile manage` -> Media -> TTS Instructions) takes plain English: **Archetype** ('a grizzled detective'), "
            "**Accent** ('standard British'), **Dynamics** ('whispering in a cavern'), **Pacing and Style** ('rapid, with a subtle smile').\n\n"
            "**Voice** (Media -> Choose TTS Voice) offers the voices of the profile's speech model. On a Gemini model that is all thirty prebuilt voices, grouped by gender and described by Google's own "
...
WIZARD_COPY = {
    "key": (
        ...
        "**Google Gemini** is the one to start with — it is the only provider here for voice and "
        "web grounding, and it draws images too. Use a **paid** (billing-enabled) key: Google may train on what a "
        "free-tier key is sent, so free-tier keys are refused for everyone but the bot's owner. **OpenRouter** opens up Claude, GPT, "
        "Llama and a few hundred others, plus image models such as FLUX and GPT Image.\n\n"
...
WIZARD_TOUR = {
    ...
    "Pictures and voice": (
        "Turn them on per character in `/profile manage` → **Media** → Image Generation and Set Speech "
        "Settings.\n\n"
        ...
        "Images can also search the web for reference photographs first — `/profile manage` → Media → Set Image Output."
    ),
...
DEFAULT_HELP_DOCS = {
    ...
    "profiles/public_hub_publishing.txt": (
        ...
        "Intro: An optional creator-written introduction of up to 300 characters, shown on the listing. Set it from `/profile manage` -> Home -> Library Intro, Edit Intro on your own listing, or Manage My Shares with one profile selected. The Library never derives one from the persona, which stays private; `/profile generate` drafts one you can keep or change. It is part of the rated content, so editing it on a rated profile makes the rating stale.\n"
        "Troubleshooting / Symptoms:\n"
    ...
    "features/image_generation.txt": (
        ...
        "Enabling: `/profile manage` -> Media -> Image Generation.\n"
        ...
        "- Symptom: 'My images stopped using web research.' Fix: Images no longer follow the profile's Grounding setting. Set `/profile manage` -> Media -> Set Image Output -> Search grounding to RAG.\n"
    ...
    "features/grounding_and_rag.txt": (
        ...
        "Images: Web Grounding applies to replies only. Image generation has its own Search grounding at `/profile manage` -> Media -> Set Image Output: RAG for any image model, or Web search on a Gemini image model.\n"
        ...
        "- Symptom: 'My images stopped using web research.' Fix: Images no longer follow Web Grounding. Set `/profile manage` -> Media -> Set Image Output -> Search grounding to RAG.\n"
